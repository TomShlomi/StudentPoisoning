...
if __name__ == "__main__":
    ...
    positive_examples = list(
        open("%s/rt-polarity.pos" % path, "r", encoding="utf-8").readlines()
    )
    positive_examples = [s.strip() for s in positive_examples]
    negative_examples = list(
        open("%s/rt-polarity.neg" % path, "r", encoding="utf-8").readlines()
    )
    ...
    w2v = gensim.models.KeyedVectors.load_word2vec_format(
        "%s/GoogleNews-vectors-negative300.bin" % path, binary=True
    )
    saved_emb = []
    for v in idx2tok:
