...
def main():
    """
    Train and evaluate a meta network for detecting trojans in target networks.
    This uses the "one class" method in the paper that compares a target network
    to a set of clean networks.
    """
    args = parser.parse_args()
    assert args.troj_type in ("patch", "blend"), "unknown trojan pattern"
    ...
    Model, input_size, class_num, inp_mean, inp_std, is_discrete = load_model_setting(
        args.task
    )
