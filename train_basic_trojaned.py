...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Specify the attack type. M: modification attack; B: blending attack.",
)


def main():
    """
    A CLI for training a basic trojaned network.
    """
    ...
    for i in range(TARGET_NUM):
        model = Model(gpu=GPU)
        atk_setting = generate_attack_spec(args.troj_type)
        trainset_mal = BackdoorDataset(
            trainset,
            atk_setting,
            apply_attack,
            choice=target_indices,
            need_pad=need_pad,
        )
        ...
        train_model(
            model,
            trainloader,
            epoch_num=int(N_EPOCH * SHADOW_PROP / TARGET_PROP),
            is_binary=is_binary,
            verbose=False,
        )
